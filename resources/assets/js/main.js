...
(function () {
  ...
  function handlePriceInput(input) {
    ...
    if (input.data('min') !== undefined && (input.data('min') != "")) {
      if (parseFloat(value) < parseFloat(input.data('min'))) {
        value = parseFloat(input.data('min'));
      }
    }
    if (input.data('max') !== undefined && (input.data('max') != "")) {
      if (parseFloat(value) > parseFloat(input.data('max'))) {
        value = input.data('max').toString();
      }
    }
    value = formatNumber(value);
    input.val(value);
    input.trigger('priceInputChange');
  }
            ...
            // Pages
            {
              ...
              templates: {
                ...
                notFound:
                  ...
                  '<p class="py-2 mb-0"><i class="ti ti-alert-circle ti-xs me-2"></i> موردی یافت نشد </p>' +
                  '</div>'
              }
            },
            // Files
