  ...
  _initRTLStyle(){
    ...
    if (!document.getElementById(cssId))
    {
      fetch('/build/manifest.json')
        ...
        .then(data => {
          const originalFileName = 'resources/css/rtl.css';
          const builtFileName = data[originalFileName].file;
          var head  = document.getElementsByTagName('head')[0];
          var link  = document.createElement('link');
          link.id   = cssId;
          link.rel  = 'stylesheet';
          link.type = 'text/css';
          link.href = window.baseUrl + 'build/'+ builtFileName
          head.appendChild(link);
        })
        .catch(error => console.error('Error getting manifest file:', error));
    }
  }
